...
//Отображение оценок при загрузке страницы
function loadScores() {
    document.querySelectorAll('.card').forEach(card => {
        const cardName = card.querySelector('h2').textContent;
        const checkedCount = localStorage.getItem(cardName);
        if (checkedCount > 0) {
            const checkboxes = document.querySelectorAll(`input[card-name="${cardName}"]`);
...
function countScores() {
    document.querySelectorAll('.card__checkbox input[type="checkbox"]').forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            ...
            localStorage.setItem(cardName, checkedCount)
        })
    });
} 
...
document.addEventListener('DOMContentLoaded', function(){
    let infoList = JSON.parse(infoListJson);
    ...
    for(let item of infoList) {
        ...
        let checkboxes = '';
        for (let i = 0; i < 5; i++) {
            checkboxes += `<input type="checkbox" card-name="${item.name}">`;
        }

        content += `<div class="card"> 
            <h2>${item.name}</h2>
            <div><span>Вселенная:</span> ${item.universe}</div>
            <div><span>Альтер эго:</span> ${item.alterego}</div>
            <div><span>Род деятельности:</span> ${item.occupation}</div>
            <div><span>Друзья:</span> ${item.friends}</div>
            <div><span>Супер силы:</span> ${item.superpowers}</div>
            <img src="${item.url}">
            <div class="card__info"><span>Описание:</span> ${item.info}</div>
            <div class="card__checkbox">${checkboxes}</div>
            </div>`;
    }
